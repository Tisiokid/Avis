...
class InterfaceAvisAU:
    def __init__(self, dossiers_csv):
        ...
        self.nouveau_branchement_var = tk.BooleanVar(value=False)
        self.extension_logement_var = tk.BooleanVar(value=False)
    ...
    def afficher_etape_chargement(self, numero_courant, numero_suivant):
        ...
        if numero_suivant:
            tk.Label(self.cadre_contenu, text=f"(Le programme pré-télécharge le dossier suivant : {numero_suivant}...)", font=("Arial", 9), bg=self.COULEURS["bg"], fg="#9E9E9E").pack(pady=5)
    ...
    def afficher_etape_avis(self, donnees, societe):
        ...
        # Réinitialisation des champs pour ce nouveau dossier
        self.press_mini_var.set("")
        self.press_tn_var.set("")
        self.press_res_var.set("")
        self.nouveau_branchement_var.set(False)
        self.extension_logement_var.set(False)
        ...
        champ_com.pack(side="left", padx=(60, 0)) # Pousse la commune vers le centre
        ...
        # CASES À COCHER (NOUVELLE SECTION)
        f_checkboxes = tk.Frame(self.cadre_contenu, bg=self.COULEURS["bg"])
        f_checkboxes.pack(pady=5)
        
        tk.Checkbutton(f_checkboxes, text="Nouveau Branchement", variable=self.nouveau_branchement_var, bg=self.COULEURS["bg"], activebackground=self.COULEURS["bg"], font=("Arial", 10, "bold")).pack(side="left", padx=20)
        tk.Checkbutton(f_checkboxes, text="Extension logement", variable=self.extension_logement_var, bg=self.COULEURS["bg"], activebackground=self.COULEURS["bg"], font=("Arial", 10, "bold")).pack(side="left", padx=20)
    ...
    def _valider_avis(self):
        ...
        # Enregistrement de l'état des cases à cocher
        self.resultat_avis["nouveau_br"] = self.nouveau_branchement_var.get()
        self.resultat_avis["ext_log"] = self.extension_logement_var.get()
...
def sauvegarder_ligne_pour_sheets(donnees, avis_aep, avis_eu, donnees_pfac, donnees_sup):
    ...
    # Écriture des nouvelles cases à cocher
    if donnees_sup:
        colonnes[5] = "True" if donnees_sup.get("nouveau_br") else ""  # F
        colonnes[6] = "True" if donnees_sup.get("ext_log") else ""     # G
    ...
    if donnees_pfac:
        colonnes[15] = donnees_pfac.get("surf_anc", "")   # P 
        colonnes[16] = donnees_pfac.get("surf_nouv", "")  # Q 
...
def traiter_avis_complet(page_avisau, donnees, chemin_dossier, config, interface):
    ...
    donnees_sup = {
        "nouveau_br": interface.resultat_avis.get("nouveau_br", False),
        "ext_log": interface.resultat_avis.get("ext_log", False)
    }
    ...
    return avis_aep, avis_eu, donnees_pfac, donnees_sup
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            while True:
                ...
                if action == "REFRESH":
                    print("🔄 Rafraîchissement de la liste des dossiers...")
                    chemin_csv = selectionner_service_et_telecharger_csv(page_avisau, config)
                    dossiers_csv = lire_csv(chemin_csv)
                ...
                for idx, d_select in enumerate(dossiers_selectionnes):
                    ...
                    if trouve:
                        ...
                        action_avis = interface.resultat_avis.get("action")
                        
                        if action_avis == "ATTENTE":
                            ...
                            stats.en_attente += 1
                        elif action_avis == "ENVOYER":
                            av_aep, av_eu, donnees_pfac, donnees_sup = traiter_avis_complet(page_avisau, donnees, dossier_p, config, interface)
